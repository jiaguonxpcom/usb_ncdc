...
extern usb_device_endpoint_struct_t g_cdcVcomDicEndpoints[USB_DEVICE_CONFIG_CDC_ACM][USB_CDC_VCOM_DIC_ENDPOINT_COUNT];
...
volatile usb_device_composite_struct_t *g_deviceComposite;
...
usb_status_t USB_DeviceCdcVcomSetConfigure(class_handle_t handle, uint8_t configure)
{
    if (USB_COMPOSITE_CONFIGURE_INDEX == configure)
    {
        for(int i=0; i<USB_DEVICE_CONFIG_CDC_ACM; i++)
        {
            ...
            g_deviceComposite->cdcVcom[i].bulkOutEndpoint = ep_vcom_dic_bulk_out[i];
            g_deviceComposite->cdcVcom[i].bulkOutEndpointMaxPacketSize = g_cdcVcomDicEndpoints[i][1].maxPacketSize;
            /* Schedule buffer for receive */
            USB_DeviceCdcAcmRecv(g_deviceComposite->cdcVcom[i].cdcAcmHandle, 
                                 g_deviceComposite->cdcVcom[i].bulkOutEndpoint,
                                 s_currRecvBuf[i], 
                                 g_deviceComposite->cdcVcom[i].bulkOutEndpointMaxPacketSize);
        }
    }
...
static void data_init(void)
{
    for(int i=0; i<USB_DEVICE_CONFIG_CDC_ACM; i++)
    {
        s_lineCoding[i][0] = (LINE_CODING_DTERATE >> 0U) & 0x000000FFU;
        s_lineCoding[i][1] = (LINE_CODING_DTERATE >> 8U) & 0x000000FFU;
        s_lineCoding[i][2] = (LINE_CODING_DTERATE >> 16U) & 0x000000FFU;
        s_lineCoding[i][3] = (LINE_CODING_DTERATE >> 24U) & 0x000000FFU;
        s_lineCoding[i][4] = LINE_CODING_CHARFORMAT;
        s_abstractState[i][0] = (STATUS_ABSTRACT_STATE >> 0U) & 0x00FFU;
        s_abstractState[i][0] = (STATUS_ABSTRACT_STATE >> 8U) & 0x00FFU;
        s_countryCode[i][0] = (COUNTRY_SETTING >> 0U) & 0x00FFU;
        s_countryCode[i][1] = (COUNTRY_SETTING >> 8U) & 0x00FFU;        
    }
}
